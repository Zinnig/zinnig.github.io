...
async function getGuildData() {
    document.getElementById("title").innerText = "Guild XP - " + guildName
    let res = {};
...
function updateChangeLeaderboard(data) {
            ...
            try {
                ...
                if (prevValue.length == 1) {
                    if (previousUpdateTime == null) {
                        ...
                        sum += player.contributed - prevValue[0].contributed;
                    } else {
                        ...
                        sum += Math.round((player.contributed - prevValue[0].contributed) / (((new Date() - previousUpdateTime) / 1000) / refreshRate));
                    }
                }
            } catch (e) {
    ...
    document.getElementById("totalgxp").innerHTML = `Total XP Gained: ${sum.toLocaleString("en")}`;
